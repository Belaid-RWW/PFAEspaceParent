...
from .forms import MarksForm, PunitionsForm, EmploisForm
from .models import Marks, Punitions, Emplois, ListEmplois
...
def teacher_marks_viewing(request):
    notes = Marks.objects.all()
    return render(request,'teacher_marks_view.html', {'notes':notes})
...
def updateMark(request, pk):
    ...
    mark=models.Marks.objects.get(id=pk)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('teacher_marks_view')

    context = {'form':form}
    return render(request, 'teacher_update_marks.html', context)
...
def deleteMark(request, pk):
    ...
    context= {'item':mark} 
    return render(request, 'teacher_delete_marks.html', context)


def admin_punitions_view(request):
    return render(request,'admin_punitions.html')

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_punitions_viewing(request):
    punitions = Punitions.objects.all()
    return render(request,'admin_punitions_view.html', {'punitions':punitions})

def AddPunition(request):
    form=PunitionsForm()
    context={'form':form}
    if request.method == 'POST':
        form=forms.PunitionsForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('admin-dashboard')
    return render(request, 'admin_add_punitions.html', context)


def updatePunition(request, pk):

    punition=models.Punitions.objects.get(id=pk)
    form = PunitionsForm(instance=punition)

    if request.method == 'POST':
        form = PunitionsForm(request.POST, instance=punition)
        if form.is_valid():
            form.save()
            return redirect('admin_punitions_view')

    context = {'form':form}
    return render(request, 'admin_update_punitions.html', context)


def deletePunition(request, pk):
    ...
    context= {'item':punition} 
    return render(request, 'admin_delete_punitions.html', context)


def admin_emplois_view(request):
    return render(request,'admin_emplois.html')

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_emplois_viewing(request):
    emplois = Emplois.objects.all()
    return render(request,'admin_emplois_view.html', {'emplois':emplois})
...
def AddEmploi(request):
    ...
    context = {'form':form}
    if request.method == 'POST':
        form=forms.EmploisForm(request.POST)
        ...
        if form.is_valid():
            form.save()
            return redirect('admin-dashboard')
    return render(request, 'admin_add_emplois.html', context)


def updateEmploi(request, pk):
    emploi=models.Emplois.objects.get(id=pk)
    form = EmploisForm(instance=emploi)
    if request.method == 'POST':
        form = EmploisForm(request.POST, instance=emploi)
        if form.is_valid():
            form.save()
            return redirect('admin_emplois_view')

    context = {'form':form}
    return render(request, 'admin_update_emplois.html', context)


def deleteEmploi(request, pk):
    ...
    context= {'item':emploi} 
    return render(request, 'admin_delete_emplois.html', context)
